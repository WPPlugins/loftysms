...
class Wp_Loftysms_Admin
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        $this->host = 'http://api.loftysms.com';
    ...
    {
        $checkUser = $this->UserSendSms($this->options['loftysms_username'], $this->options['loftysms_password']);
    ...
    public function update_loftysms_details()
    {
        if (current_user_can('activate_plugins')) {
            register_setting($this->plugin_name, $this->plugin_name, array($this, 'LinkLoftysmsAccount'));
        }
        $checkUser = $this->LoftysmsUser($this->options['loftysms_username'], $this->options['loftysms_password']);
        $code = wp_remote_retrieve_response_code($checkUser);
        $message = wp_remote_retrieve_body($checkUser);
        if ($code == 200) {
            add_settings_error(
    ...
    public function LoftysmsUser($username, $password)
    {
    ...
    public function UserSendSms($username, $password)
    {
        $headers = $this->headers($username, $password);
        $args = ['headers' => $headers];
        $url = '/me/sendsms';
    ...
    public function loftysms_sendsms()
    {
        ...
        wp_redirect(admin_url() . '?page=' . $this->plugin_name . '&code=' . $code . '&message=' . $message);
    }
}
